...
class AI_Service:
    ...
    def process_frame(self, frame: np.ndarray, frame_count: int) -> np.ndarray:
        ...
        self.note_annotator.annotate(frame) #draw legend
        
        # Vehicle detection
        detect_results = self.vehicle_detector(frame, conf=0.6, verbose = False)
    
        if detect_results[0]:
            dets = []
            for x1, y1, x2, y2, conf, id in detect_results[0].boxes.data.cpu().numpy():
